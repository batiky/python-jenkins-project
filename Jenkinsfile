pipeline{
    ...
    stage('create-cluster'){
        ...
        stage('apply-k8s'){
            ...
            steps{
                script {
                    env.EBS_VOLUME_ID = sh(script:"aws ec2 describe-volumes --region us-east-1d --filters Name=tag:Name,Values='k8s-python-mysql-app' | grep VolumeId |cut -d '\"' -f 4| head -n 1", returnStdout: true).trim()
                }
